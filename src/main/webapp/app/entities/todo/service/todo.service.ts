...
export class TodoService {
  ...
  protected convertDateFromClient(todo: ITodo): ITodo {
    return Object.assign({}, todo, {
      createdAt: todo.createdAt?.isValid() ? todo.createdAt.toJSON() : undefined,
      deadline: todo.deadline?.isValid() ? todo.deadline.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((todo: ITodo) => {
        todo.createdAt = todo.createdAt ? dayjs(todo.createdAt) : undefined;
        todo.deadline = todo.deadline ? dayjs(todo.deadline) : undefined;
      });
    }
    return res;
  }
